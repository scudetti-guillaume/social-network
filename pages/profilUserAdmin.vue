<template>
  <v-card class="card">
    <v-card-text class="card-profil-title-admin">
      ...
      <button id="btn-flag-admin" v-if="this.userReport.length > 0 && this.userBan == false"
        @click="(showBan = !showBan), banUserId(userid)"> Bannir
      ...
      <button id="btn-flag-admin-ban" v-else-if="this.userReport.length > 0 && this.userBan == true"
        @click="(showBan = !showBan), banUserId(userid)"> Banni
        <div id="flag-admin-signal-profil"><v-icon id="flag-admin-signal-icon-profil"
            size="30px">mdi-check-underline-circle</v-icon>
        </div>
      </button>
      <button v-else id="btn-flag-admin-all" @click="(showBan = !showBan), banUserId(userid)">Bannir <div
          id="flag-admin-signal-profil"><v-icon id="flag-admin-signal-icon-profil">mdi-flag</v-icon>
          <div class="buble-report-profil">
          </div>
        </div></button>
    </v-card-text>
    ...
    <v-card-text class="card-profil-biographie">
      ...
      <p v-if="this.bioUser == '' || this.bioUser == undefined" class="card-profil-biographie-p">{{ biographiePadmin }}
      </p>
      <p v-else class="card-profil-biographie-p">{{ bioUser }}</p>


      <button v-if="this.bioUser !== '' && !modifbio && this.bioUser !== undefined" class="btn-bio-mod"
        @click="getBio(), (warningDelete = !warningDelete)">
    ...
    <v-card-text v-if="follower[0] !== undefined" class="card-profil-friend">
      ...
      <div v-for="(p, index) in info" class="btn-profil-follow">
        ...
        <button v-if="p._id == userjwtid" class="btn-unfollow-admin " @click="getUnFollowBack(userid)"> Se
          désabonner</button>
      </div>
    ...
    <v-card-text class="card-profil-post" v-if="pub[0] != undefined">
      ...
      <div class="align-pub-user">
        <div class="btn-profil-post-del" v-for="(p, index) in pub">
          ...
          <div v-if="p.likers.length > 0" class="like-profilmain-user"><v-icon
              class="img-like-profilmain">mdi-thumb-up-outline</v-icon><span
              id="span-img-like-profilmain">{{ p.likers.length }}</span></div>
    ...
    <WarningDeletePicture v-if="warningDelPic" v-show="warningDelPic" @close-modale-delpicuser=" warningDelPic = false"
      @close-modale-delpicuser-confirm="warningDelPic = false, deletePicUser()" />
    <WarningRecord v-if="warningRecord" v-show="warningRecord" @close-modale-record="warningRecord = false"
      @close-modale-record-confirm="warningRecord = false, modifbio = false" />
    <WarningEmpty v-if="warningEmpty" v-show="warningEmpty" @close-modale-empty="warningEmpty = false" />
    <WarningDelete v-if="warningDelete" v-show="warningDelete" @close-modale-biodelete="warningDelete = false"
      @close-modale-biodelete-confirm="warningDelete = false, deleteUserBio()" />
